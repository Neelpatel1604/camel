...
<Card title="MemantoToolkit Integration" icon="warehouse">
Use [Memanto](https://docs.memanto.ai) for explicit long-term memory operations
through `ChatAgent` tools: `memanto_remember`, `memanto_recall`, and
`memanto_answer`.
...
agent = ChatAgent(
    system_message="You manage long-term memory.",
...
Session activation and one retry on an expired session are handled internally.
Chat history continues to use the agent's configured memory backend.

See the [toolkit reference](/reference/camel.toolkits.memanto_toolkit).
</Card>
</Card>
